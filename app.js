...
function normalizeEntry(raw) {
  ...
  if (typeof raw === "string") return { status: raw, amount: null };
  return raw;
}
  ...
  const perMed = meds.map((med) => {
    ...
    for (const mk of Object.keys(aggregated)) {
      ...
      for (const day of Object.keys(rec)) {
        const status = normalizeEntry(rec[day])?.status;
        if (status === "taken") taken++; else if (status === "missed") missed++;
      }
    }
...
function toCSV(meds, aggregated) {
  ...
  const rows = [["Date", "Medication", "Category", "Status", "Amount"]];
  Object.keys(aggregated).sort().forEach((mk) => {
    ...
    Object.keys(rec).forEach((medId) => {
      ...
      Object.keys(rec[medId]).sort((a, b) => Number(a) - Number(b)).forEach((day) => {
        const entry = normalizeEntry(rec[medId][day]);
        rows.push([`${y}-${String(m).padStart(2, "0")}-${String(day).padStart(2, "0")}`, med.name, catLabel, entry?.status || "", entry?.amount || ""]);
      });
    });
  });
...
function MedTracker() {
  ...
  const [amountTarget, setAmountTarget] = useState(null); // { date, medId, medName, color }
  ...
  const getStatus = (date, medId) => {
    ...
    return normalizeEntry(recordsByMonth[mk]?.[medId]?.[date.getDate()])?.status;
  };
  const getAmount = (date, medId) => {
    const mk = monthKey(date.getFullYear(), date.getMonth());
    return normalizeEntry(recordsByMonth[mk]?.[medId]?.[date.getDate()])?.amount || null;
  };
  const toggleCell = (date, medId) => {
    ...
    setRecordsByMonth((prev) => {
      ...
      const current = normalizeEntry(medRec[day]);
      let nextEntry;
      if (!current?.status) nextEntry = { status: "taken", amount: current?.amount ?? null };
      else if (current.status === "taken") nextEntry = { status: "missed", amount: current.amount ?? null };
      else nextEntry = null;
      if (nextEntry) medRec[day] = nextEntry; else delete medRec[day];
  ...
  const setAmount = (date, medId, amountStr) => {
    const mk = monthKey(date.getFullYear(), date.getMonth());
    const day = date.getDate();
    ...
    setRecordsByMonth((prev) => {
      const monthRec = { ...(prev[mk] || {}) };
      const medRec = { ...(monthRec[medId] || {}) };
      const existing = normalizeEntry(medRec[day]);
      let nextEntry;
      if (!trimmed && !existing) nextEntry = null;
      else if (!trimmed) nextEntry = existing.status ? { status: existing.status, amount: null } : null;
      else nextEntry = { status: existing?.status === "missed" ? "missed" : "taken", amount: trimmed };
      if (nextEntry) medRec[day] = nextEntry; else delete medRec[day];
      monthRec[medId] = medRec;
      const next = { ...prev, [mk]: monthRec };
      persistRecords(mk, monthRec);
  ...
  const openAmountEditor = (date, medId, medName, color) => {
    const existing = getAmount(date, medId);
    const med = (meds || []).find((m) => m.id === medId);
    setAmountValue(existing || med?.dose || "");
    setAmountTarget({ date, medId, medName, color });
  };
  const closeAmountEditor = () => { setAmountTarget(null); setAmountValue(""); };
  const saveAmount = () => {
    if (!amountTarget) return;
    setAmount(amountTarget.date, amountTarget.medId, amountValue);
    closeAmountEditor();
  };
  const handleCellPointerDown = (date, medId, medName, color) => {
    longPressRef.current.fired = false;
    longPressRef.current.timer = setTimeout(() => {
      longPressRef.current.fired = true;
      openAmountEditor(date, medId, medName, color);
    }, 450);
  };
  const handleCellPointerUp = () => { clearTimeout(longPressRef.current.timer); };
  const handleCellClick = (date, medId) => {
    if (longPressRef.current.fired) { longPressRef.current.fired = false; return; }
    toggleCell(date, medId);
  };
  ...
  const labelWidthPx = viewMode === "week" ? 90 : 108;
  ...
  return (
    <div style={pageStyle}>
                        ...
                        return (
                          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: `${cellGap}px`, marginBottom: viewMode === "week" ? "0.6rem" : "0.5rem" }}>
                            <div style={{ width: `${labelWidthPx}px`, flexShrink: 0, paddingRight: "0.5rem", position: "sticky", left: 0 }}>
                              <div style={{ fontSize: viewMode === "week" ? "0.8rem" : "0.72rem", fontWeight: 600, color: "#F4F1EC", lineHeight: 1.2, wordBreak: "break-word" }} title={m.name}>{m.name}</div>
                              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.6rem", color: "#8A857B" }}>{pct === null ? "—" : `${pct}%`}</div>
                            </div>
                            {daysList.map((d, di) => {
                              const status = getStatus(d, m.id); const amt = getAmount(d, m.id); const isToday = sameDate(d, today);
                              return (
                                <button key={di}
                                  onClick={() => handleCellClick(d, m.id)}
                                  onPointerDown={() => handleCellPointerDown(d, m.id, m.name, color)}
                                  onPointerUp={handleCellPointerUp}
                                  onPointerLeave={handleCellPointerUp}
                                  aria-label={`${m.name} ${d.toDateString()}: ${status || "not tracked"}${amt ? `, ${amt}` : ""}. Hold to log amount.`}
                                  style={{ width: `${cellPx}px`, height: `${cellPx}px`, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", border: isToday ? `1.5px solid ${color}` : "1px solid rgba(244,241,236,0.08)", borderRadius: `${cellRadius}px`, background: status === "taken" ? color : status === "missed" ? "rgba(244,241,236,0.03)" : "rgba(244,241,236,0.045)", boxSizing: "border-box", cursor: "pointer", padding: 0, transition: "background 0.15s ease", touchAction: "manipulation", userSelect: "none", WebkitUserSelect: "none" }}>
                                  ...
                                  {amt && <span style={{ position: "absolute", top: "2px", right: "2px", width: "5px", height: "5px", borderRadius: "50%", background: status === "taken" ? "#1A1B22" : color, opacity: 0.85 }} />}
                                </button>
                              );
                            })}
                          </div>
      ...
      {amountTarget && (
        <div style={modalOverlayStyle} onClick={closeAmountEditor}>
          <div onClick={(e) => e.stopPropagation()} style={modalStyle}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.3rem" }}>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: "1.1rem", fontWeight: 600, margin: 0, color: "#F4F1EC" }}>{amountTarget.medName}</h2>
              <button onClick={closeAmountEditor} style={{ background: "none", border: "none", color: "#A6A197", cursor: "pointer" }}><XIcon size={20} /></button>
            </div>
            <div style={{ fontSize: "0.75rem", color: "#A6A197", marginBottom: "1rem", fontFamily: "'JetBrains Mono', monospace" }}>{amountTarget.date.toDateString()}</div>
            <label style={labelStyle}>How much did you take?</label>
            <input autoFocus value={amountValue} onChange={(e) => setAmountValue(e.target.value)} placeholder="e.g. 500mg, 1 tablet, 2 puffs" style={inputStyle} onKeyDown={(e) => e.key === "Enter" && saveAmount()} />
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.9rem" }}>
              <button onClick={() => { setAmountValue(""); saveAmount(); }} style={{ ...exportBtnStyle, color: "#A6A197" }}>Clear</button>
              <button onClick={saveAmount} style={{ flex: 1, background: amountTarget.color, color: "#1A1B22", border: "none", borderRadius: "10px", padding: "0.65rem", fontWeight: 700, fontSize: "0.85rem", cursor: "pointer" }}>Save</button>
            </div>
          </div>
        </div>
      )}
